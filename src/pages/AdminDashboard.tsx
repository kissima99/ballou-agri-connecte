      ...
      <div className="container px-4 py-12 mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 font-medium">Gestion centralisée des flux Ballou-Dakar</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative">
              <Button variant="outline" className="rounded-full h-12 w-12 p-0 border-orange-200 bg-white">
                <Bell className="h-6 w-6 text-orange-600" />
                {stats.pending > 0 && (
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold h-5 w-5 flex items-center justify-center rounded-full animate-bounce">
                    {stats.pending}
                  </span>
                )}
              </Button>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                ...
                <div className="bg-blue-100 p-3 rounded-2xl">
                  <PackageCheck className="text-blue-600 h-6 w-6" />
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-none shadow-xl bg-white overflow-hidden rounded-3xl">
          <CardHeader className="border-b bg-stone-50/50 py-6">
            <div className="flex flex-col md:flex-row justify-between gap-4">
              <CardTitle className="text-xl font-bold">Flux de Commandes Récentes</CardTitle>
              <div className="flex gap-2">
                <div className="relative w-full md:w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input 
                    placeholder="Rechercher BAC-XXXX..." 
                    className="pl-10 h-10 rounded-xl border-stone-200"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
                </div>
                <Button variant="outline" className="rounded-xl h-10 border-stone-200">
            ...
            <Table>
              <TableHeader className="bg-stone-50">
                <TableRow className="hover:bg-transparent border-stone-100">
                  <TableHead className="font-bold text-gray-900 py-4 pl-6">ID Commande</TableHead>
                  <TableHead className="font-bold text-gray-900">Client</TableHead>
                  <TableHead className="font-bold text-gray-900">Produits</TableHead>
                  ...
                  filteredOrders.map((order) => (
                    <TableRow key={order.id} className="border-stone-50 hover:bg-stone-50/50 transition-colors">
                      ...
                      <TableCell className="max-w-[250px]">
                        <p className="text-xs text-gray-600 truncate font-medium">{order.product}</p>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge 
                          className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider border-none ${
                      ...
                      <TableCell className="text-right pr-6">
                        <div className="flex justify-end gap-2">
                          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-orange-50 hover:text-orange-600">
                            <Eye className="h-4 w-4" />
                          </Button>
                          {order.status === 'Payé' && (
                            <Button 
                              onClick={() => updateOrderStatus(order.id, 'En cours')}
                              className="bg-blue-600 hover:bg-blue-700 h-8 px-3 text-[10px] font-bold rounded-lg"
                            >
                              EXPÉDIER
                            </Button>
                          )}
                          {order.status === 'En cours' && (
                            <Button 
                              ...
                              className="bg-green-600 hover:bg-green-700 h-8 px-3 text-[10px] font-bold rounded-lg"
                            >
                              LIVRER
                            </Button>
                          )}
